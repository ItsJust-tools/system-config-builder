...
if (!pkg) {
  ...
  if (pkg.name === "template" || pkg.name === "@itsjust/template") {
    warn(
      `package.json name is still "${pkg.name}" — rename to your tool scope`,
    );
  } else {
...
if (pkg) {
  ...
  const needsImageExport =
    /['"]png['"]|['"]jpeg['"]|['"]webp['"]|['"]pdf['"]/.test(toolConfigTxt);

  if (needsImageExport && !(hasHtml2canvas && hasJsPdf)) {
    warn(
      "Image/PDF export formats declared but html2canvas/jspdf not in dependencies",
    );
...
if (errors.length === 0 && warnings.length === 0) {
  ...
  console.log(
    `⚠️  Preflight passed with ${warnings.length} warning(s). Review before shipping.`,
  );
  process.exit(0);
} else {
  console.log(
